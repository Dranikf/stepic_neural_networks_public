...
class SimpleCarAgent(Agent):
    ...
    def choose_action(self, sensor_info):
        ...
        self.reward_history.append(0.0) # мы пока не знаем, 
                                        # какая будет награда, это
    ...
    def receive_feedback(self, reward, train_every=50, reward_depth=7):
        """
        Получить реакцию на последнее решение, 
        принятое сетью, и проанализировать его
        
        :param reward: оценка внешним миром наших действий
        :param train_every: сколько нужно собрать наблюдений, 
                            прежде чем запустить обучение на несколько эпох
        :param reward_depth: на какую глубину по времени распространяется
                             полученная награда
        """
        ...
        # Если у нас накопилось хоть чуть-чуть данных, 
        # давайте потренируем нейросеть прежде чем собирать новые данные
        # (проверьте, что вы в принципе храните достаточно данных 
        # (параметр `history_data` в `__init__`),
        # чтобы условие len(self.reward_history) >= train_every выполнялось
        if  not self.evaluate_mode and \
            (len(self.reward_history) >= train_every) and\
            not (self.step % train_every):
            ...
            train_data = \
                [(x[:, np.newaxis], y) for x, y in zip(X_train, y_train)]
            
            
            self.neural_net.SGD(
                training_data=train_data, epochs=500, 
                mini_batch_size=int(train_every/4), eta=0.001)
